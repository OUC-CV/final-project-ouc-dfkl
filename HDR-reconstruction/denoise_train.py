...
from datadenoise import prepare_data, Dataset
from utils import *
...
def main():
    ...
    dataset_val = Dataset(train=False) if not opt.preprocess else None
    ...
    for epoch in range(opt.epochs):
        ...
        for i, data in enumerate(loader_train, 0):
            ...
            # results
            net.eval()
            out_train = torch.clamp(imgn_train-net(imgn_train), 0., 1.)
        ...
        if dataset_val is not None:
            ...
            for k in range(len(dataset_val)):
                ...
                img_val, imgn_val = Variable(img_val), Variable(imgn_val)
                out_val = torch.clamp(imgn_val-net(imgn_val), 0., 1.)
